    ...
    <div className="py-8 bg-primary-100">
      <Container>
        <div className="w-full p-2 bg-gradient-to-b from-white to-primary-200 drop-shadow-lg rounded-lg pb-4">
            ...
            {isAuthor && (
              <div className="absolute right-6 top-6">
                <Link to={`/edit-post/${post.$id}`}>
                  <NmButton className="mr-3">Edit</NmButton>
                </Link>
                <NmButton onClick={deletePost}>Delete</NmButton>
              </div>
            )}
          </div>

          <div className="w-full mb-6 rounded-l p-4 ">
            <h1 className="text-4xl text-center drop-shadow-lg font-bold ">
              {post.title}
            </h1>
          </div>
          ...
          <div className='p-2 mt-2 flex justify-center gap-4 text-primary-500 text-xl '>
            <h1> {titleCase(post.author)}</h1>
            *
          <h1>{ titleCase(post.createdOn) }</h1>
          </div>
        </div>
      </Container>
    </div>
